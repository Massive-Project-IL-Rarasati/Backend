...
export const deleteKucing = async (req, res) => {
  ...
  try {
    const result = await query("DELETE FROM kucing WHERE cat_id = ?", [cat_id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ msg: "Kucing tidak ditemukan" });
    }
    res.status(200).json({ success: true, msg: "Kucing berhasil dihapus" });
  } catch (error) {
    console.error("Terjadi kesalahan", error);
    res.status(500).json({ msg: "Terjadi kesalahan pada server" });
  }
